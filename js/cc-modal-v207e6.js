function createModal() {
	...
	const baseUrl = window.object_name.templateUrl;
	const logo = window.object_name.global_logo || `${baseUrl}/assets/images/icons/logo.svg`;
...
async function openPageInModalWindow(event) {
  ...
  if (!link) return;
	const slug = link.dataset.slug || link.href.split('/')[4];
  ...
  const preventReachModal = !!window.object_name.prevent_reach_modal;
...
function checkShouldOpenModal(slug) {
  const alwaysModalSlugs = ['reach', 'size-chart'];
  const toggleModal = window.object_name.toggle_modal;
  const postType = window.object_name.post_type;
  ...
  if (alwaysModalSlugs.includes(slug)) {
    return true;
  }
...
function shouldOpenInNewWindow() {
  return window.object_name && window.object_name.target_blank;
}
